...
function filterHistory(data: InstitutionalResponse | null, window: string) {
  ...
  return {
    ...data,
    history: data.history.slice(-sizes[window])
  };
}
  ...
  return (
    <div className="space-y-5">
      <header className="flex flex-wrap items-end justify-between gap-4 border-b border-line pb-4">
        ...
        {data ? <BiasPill value={data.bias} /> : null}
      </header>
      ...
      <Panel>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <select
            ...
            className="h-10 border-line bg-white text-sm"
          >
            {assets.map((item) => (
          ...
          <div className="flex flex-wrap gap-2">
            {windows.map((item) => (
              <button
                ...
                className={`h-9 border px-3 text-sm ${window === item ? "border-ink bg-ink text-white" : "border-line bg-white text-muted"}`}
              >
                {item}
              </button>
            ))}
      ...
      {loading || !filtered ? (
        <Panel>Loading positioning data...</Panel>
      ) : (
        <>
          <section className="grid gap-3 md:grid-cols-3 xl:grid-cols-6">
            <StatCard label="Long %" value={filtered.long_percentage} />
            <StatCard label="Short %" value={filtered.short_percentage} />
            <StatCard label="Net Position" value={filtered.net_position} />
            <StatCard label="Weekly Change" value={filtered.weekly_change} />
            <StatCard label="4 Week Velocity" value={filtered.four_week_velocity} />
            <StatCard label="Score" value={filtered.score} />
          </section>

          <Panel>
            <SectionTitle title={`${filtered.asset} Positioning Trends`} />
            ...
            <div className="flex items-center gap-4">
              <div className="h-3 flex-1 border border-line bg-canvas">
                <div
                  ...
                  style={{ width: `${Math.max(0, Math.min(filtered.position_percentile, 100))}%` }}
                />
              </div>
              <div className="w-20 text-right text-sm font-semibold">
                {formatNumber(filtered.position_percentile)}%
              </div>
            </div>
          </Panel>
        </>
