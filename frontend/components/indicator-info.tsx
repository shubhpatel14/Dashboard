  ...
  return (
    <>
      <button
        ...
        aria-label={`Open ${indicator.name} explanation`}
        title={`Open ${indicator.name} explanation`}
      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" role="dialog" aria-modal="true">
          <div className="max-h-[86vh] w-full max-w-2xl overflow-y-auto rounded-xl border border-line bg-surface p-5 shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b border-line pb-4">
              <div>
                <div className="text-xs font-semibold uppercase text-muted">Indicator Intelligence</div>
                <h2 className="mt-1 text-xl font-semibold text-ink">{indicator.name}</h2>
              </div>
              <button
                type="button"
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              ...
              <div className="rounded-lg border border-line bg-canvas p-3">
                <div className="text-xs font-semibold uppercase text-muted">Risk Signal</div>
            ...
            <div className="mt-5 space-y-4 text-sm leading-6 text-ink">
              <section>
                <h3 className="text-xs font-semibold uppercase text-muted">Meaning</h3>
                <p>{indicator.measures || `${indicator.name} is a macro input used by the scoring model.`}</p>
              </section>
              <section>
                <h3 className="text-xs font-semibold uppercase text-muted">Why It Matters</h3>
                <p>{indicator.info || `${indicator.name} can shift policy expectations, yields, USD, liquidity, and risk appetite.`}</p>
              </section>
              <section>
                <h3 className="text-xs font-semibold uppercase text-muted">Current Reading Interpretation</h3>
                <p>{indicator.explanation || indicator.market_impact || "Current interpretation is neutral due to limited detail."}</p>
              </section>
              <section>
                <h3 className="text-xs font-semibold uppercase text-muted">Market Impact</h3>
                <p>{indicator.market_impact || indicator.impact || "Used as part of the broader macro score."}</p>
              </section>
              <section className="grid gap-2 sm:grid-cols-2">
                <div>
                  <h3 className="text-xs font-semibold uppercase text-muted">Formula</h3>
